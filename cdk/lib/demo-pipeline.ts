...
export class DemoStack extends cdk.Stack {
  ...
  constructor(scope: cdk.Construct, id: string, props?: cdk.StackProps) {
        ...
        phases: {
          ...
          post_build: {
            commands: [
              ...
              "echo image $ECR_REPO_URI:$TAG",
              'kubectl set image deployment '+'app'+' '+'app'+'='+'$ECR_REPO_URI:$TAG',
              "sleep 60",
    ...
    this.ecrRepo.grantPullPush(cdkBuild.role!)
    cluster.awsAuth.addMastersRole(cdkBuild.role!)
    cdkBuild.addToRolePolicy(new iam.PolicyStatement({
      actions: ['eks:DescribeCluster'],
      resources: [`${cluster.clusterArn}`],
    }))
